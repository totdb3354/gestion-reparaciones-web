...
- [x] Menú contextual SUPERTECNICO: "Editar" (solo R/G), "Borrar", separador, "📋  Copiar celda", separador, "Añadir incidencia" (solo si no tiene incidencia), "Cancelar incidencia" (solo con incidencia abierta). TECNICO y ADMIN: solo "Copiar celda".
- [x] (sub-proyecto 2) "Editar" abre el formulario en modo edición: navega a `/reparaciones/historial/editar/<idRep>` (en Glass, `/reparaciones/historial/glass/editar/<idRep>`), diálogo sobre la tabla (ficha `formulario.md`). Al cerrarlo se recarga la tabla, como el `cargarDatos` del JavaFX.
