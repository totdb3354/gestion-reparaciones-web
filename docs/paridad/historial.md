...
- [x] Tablas sin ordenación por clic ni reordenación; orden = el que devuelve el servidor (fecha de asignación descendente y, a igual fecha, número de ID descendente); fila seleccionada azul medio; separador `fila-sep`; celdas 12 px.
...
- [x] Fechas en dos líneas: asignación (10 px, #9AA0AA) y "→ fin" (11 px, azul medio), formato `yyyy/MM/dd` (sin hora); "—" si falta.
...
- [x] Filas: borde izquierdo de 8 px `fila-incidencia-brd` con incidencia abierta, `fila-reparado-brd` con incidencia resuelta, transparente si no.
...
- "Editar modelo": el JavaFX abre el diálogo con el modelo actual ya seleccionado ("Guardar" habilitado al instante); la web lo resalta y habilita "Guardar" al elegir un modelo.
